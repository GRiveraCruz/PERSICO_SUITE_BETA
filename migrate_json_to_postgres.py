...
def _canonical_hash(rec):
    ...
    mismo hash, sin importar el orden de las llaves del dict)."""
...
def ejecutar_migracion(session, rep, solo=None):
    ...
    if incluir("work_hours"):
        migrar_por_anio(session, rep, "work_hours", Path(DATA_DIR) / "WHs", r"^wh_(\d{4})\.json$",
                         db.WorkHour, "id",
                         lambda r: {"employee": r.get("employee"), "job": r.get("work_code") or r.get("job"),
    ...
    if incluir("areas"):
        migrar_lista_simple(session, rep, "areas", "PERSONAL/areas.json", db.Area, "nombre", lambda r: {"nombre": r.get("nombre")})
    if incluir("perfiles"):
    ...
    if incluir("permisos"):
        migrar_lista_simple(session, rep, "permisos", "PERSONAL/permisos.json", db.Permiso, "pid",
    ...
    if incluir("reassign_orders"):
        migrar_lista_simple(session, rep, "reassign_orders", "reassign_orders.json", db.ReassignOrder, "folio", lambda r: {})
    if incluir("recovery"):
    ...
    if incluir("ordenes_servicio"):
        migrar_lista_simple(session, rep, "ordenes_servicio", "ordenes_servicio.json", db.OrdenServicio, "id",
                             lambda r: {"os_id": r.get("id"), "estatus": r.get("estatus")})
    if incluir("tareas_asignadas"):
        migrar_lista_simple(session, rep, "tareas_asignadas", "tareas_asignadas.json", db.TareaAsignada, "id",
                             lambda r: {"ta_id": r.get("id"), "estatus": r.get("estatus")})

    if incluir("esquemas_tributarios"):
        migrar_lista_simple_por_hash(session, rep, "esquemas_tributarios", "FINANZAS/esquemas_tributarios.json", db.EsquemaTributario, lambda r: {})
    if incluir("recepciones"):
        migrar_lista_simple_por_hash(session, rep, "recepciones", "FINANZAS/recepciones.json", db.Recepcion, lambda r: {"job": r.get("job")})
    if incluir("procesar_compra"):
        migrar_lista_simple_por_hash(session, rep, "procesar_compra", "FINANZAS/procesar_compra.json", db.ProcesarCompra, lambda r: {"job": r.get("job")})
    if incluir("cpp"):
        migrar_lista_simple(session, rep, "cpp", "FINANZAS/cpp.json", db.CPP, "cpp_number", lambda r: {})
    if incluir("pagos"):
        migrar_lista_simple(session, rep, "pagos", "FINANZAS/pagos.json", db.Pago, "folio", lambda r: {})
    if incluir("cpc"):
    ...
    if incluir("project_configs"):
        migrar_lista_simple(session, rep, "project_configs", "project_configs.json", db.ProjectConfig, "job", lambda r: {})
    if incluir("ingresos"):
        migrar_lista_simple_por_hash(session, rep, "ingresos", "ingresos.json", db.Ingreso, lambda r: {"job": r.get("job")})
    if incluir("apartados"):
        migrar_lista_simple_por_hash(session, rep, "apartados", "apartados.json", db.Apartado, lambda r: {"job": r.get("job")})
    if incluir("salidas"):
        migrar_lista_simple_por_hash(session, rep, "salidas", "salidas.json", db.Salida, lambda r: {"job": r.get("job")})
    if incluir("viaticos"):
        migrar_lista_simple_por_hash(session, rep, "viaticos", "viaticos.json", db.Viatico, lambda r: {"job": r.get("job")})
    if incluir("gastos_viaje"):
        migrar_lista_simple_por_hash(session, rep, "gastos_viaje", "gastos_viaje.json", db.GastoViaje, lambda r: {"job": r.get("job")})
    if incluir("envios"):
        migrar_lista_simple_por_hash(session, rep, "envios", "envios.json", db.Envio, lambda r: {"job": r.get("job")})
    ...
    if incluir("proveedores"):
        migrar_lista_simple_por_hash(session, rep, "proveedores", "proveedores.json", db.Proveedor, lambda r: {"nombre": r.get("nombre")})
    if incluir("generated_pos"):
        migrar_lista_simple(session, rep, "generated_pos", "generated_pos.json", db.GeneratedPO, "folio", lambda r: {})
