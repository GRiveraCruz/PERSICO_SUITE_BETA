...
v2: incluye columnas source_id/year_key/content_hash agregadas tras pruebas
reales contra PostgreSQL (ver conversacion del proyecto de migracion).
...
def upgrade() -> None:
    ...
    op.create_index('ix_fx_rates_fecha', 'fx_rates', ['fecha'])
    ...
    op.create_table('project_configs',
        sa.Column('job', sa.String(), nullable=False, unique=True),
        sa.Column('id', sa.Integer(), primary_key=True),
        sa.Column('data', postgresql.JSONB(), nullable=False),
        sa.Column('created_at', sa.DateTime()),
        sa.Column('updated_at', sa.DateTime()),
    ...
    op.create_index('ix_ingresos_job', 'ingresos', ['job'])
    ...
    op.create_index('ix_apartados_job', 'apartados', ['job'])
