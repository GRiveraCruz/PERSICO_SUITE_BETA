...
function pcRenderPersonalTable() {
  ...
  const tb = document.getElementById('pc-personal-body');
  const thead = document.getElementById('pc-personal-thead');
  if(!tb || !thead) return;

  // Preservar lo ya capturado en pantalla (si lo hay) antes de reconstruir columnas
  const rowsSource = tb.rows.length ? pcGetPlanPersonalData() : (pcPersonalSaved||[]);
  ...
  const sb = 'background:var(--card)';
  thead.innerHTML = `
    <th style="position:sticky;left:0;${sb};z-index:2;min-width:170px">Person</th>
    <th style="position:sticky;left:170px;${sb};z-index:2;width:150px">Área</th>
    <th style="position:sticky;left:320px;${sb};z-index:2;width:80px;text-align:right">Rate</th>
    <th style="position:sticky;left:400px;${sb};z-index:2;width:90px;text-align:right">Total Hours</th>
    <th style="position:sticky;left:490px;${sb};z-index:2;width:100px;text-align:right;border-right:2px solid var(--border)">Total Amount</th>
    ${days.map(dt => `<th style="width:44px;text-align:center;font-size:9px;color:${[0,6].includes(dt.getDay())?'var(--muted)':'var(--muted2)'}">${dt.toLocaleDateString('es-MX',{day:'2-digit',month:'2-digit'})}</th>`).join('')}
    <th style="position:sticky;right:0;${sb};width:32px"></th>
  `;
...
function pcAddPersonaRow(data={}) {
  ...
  const sb = 'background:var(--bg)';
  const tr = document.createElement('tr');
  tr.id = rowId;
  const daysVal = data.days || {};
  const selectedArea = (data.area || '').toUpperCase();
  const areaOptions = ['<option value="">—</option>']
    .concat(PC_PERSONAL_AREAS.map(a => `<option value="${a}" ${a===selectedArea?'selected':''}>${a.replace('INGENIERIA','Ing.').split(' ').map(w=>w.charAt(0)+w.slice(1).toLowerCase()).join(' ')}</option>`));
  ...
  tr.innerHTML = `
    <td style="position:sticky;left:0;${sb};z-index:1">
      <select data-field="employee" onchange="pcPersonaRowRecalc('${rowId}')" style="width:100%;background:var(--inp);border:1px solid var(--border);border-radius:4px;color:var(--text);padding:4px 6px;font-size:11px">
        ${options.join('')}
      </select>
    </td>
    <td style="position:sticky;left:170px;${sb};z-index:1">
      <select data-field="area" onchange="pcUpdatePersonalTotals()" style="width:100%;background:var(--inp);border:1px solid var(--border);border-radius:4px;color:var(--amber);padding:4px 6px;font-size:11px">
        ${areaOptions.join('')}
      </select>
    </td>
    <td style="position:sticky;left:320px;${sb};z-index:1;text-align:right;font-family:'DM Mono',monospace" id="${rowId}-rate">—</td>
    <td style="position:sticky;left:400px;${sb};z-index:1;text-align:right;font-family:'DM Mono',monospace;font-weight:700" id="${rowId}-hours">0</td>
    <td style="position:sticky;left:490px;${sb};z-index:1;text-align:right;font-family:'DM Mono',monospace;font-weight:700;color:var(--gold);border-right:2px solid var(--border)" id="${rowId}-amount">$0.00</td>
    ${pcPersonalDayKeys.map(key => `<td style="padding:2px"><input data-day="${key}" type="number" min="0" step="0.5" value="${daysVal[key]||''}" oninput="pcPersonaRowRecalc('${rowId}')" style="width:40px;background:var(--inp);border:1px solid var(--border);border-radius:3px;color:var(--text);padding:3px;font-size:10px;text-align:center"></td>`).join('')}
    <td style="position:sticky;right:0;background:var(--card)"><button onclick="this.closest('tr').remove();pcUpdatePersonalTotals()" style="background:none;border:none;color:var(--muted);cursor:pointer;font-size:13px">✕</button></td>
  `;
  tb.appendChild(tr);
...
function pcPersonaRowRecalc(rowId) {
  ...
  if(!tr) return;
  const emp  = tr.querySelector('[data-field="employee"]')?.value || '';
  const rate = Object.prototype.hasOwnProperty.call(pcPersonalRatesMap, emp) ? pcPersonalRatesMap[emp] : null;
  const hours = [...tr.querySelectorAll('[data-day]')].reduce((s,inp)=>s+(parseFloat(inp.value)||0),0);
...
function pcUpdatePersonalTotals() {
  ...
  [...tb.rows].forEach(tr => {
    const emp = tr.querySelector('[data-field="employee"]')?.value || '';
    const area = (tr.querySelector('[data-field="area"]')?.value || '').toUpperCase();
  ...
  if(areaTb) {
    const label = a => a.replace('INGENIERIA','Ing.').split(' ').map(w=>w.charAt(0)+w.slice(1).toLowerCase()).join(' ');
    areaTb.innerHTML = PC_PERSONAL_AREAS.map(a => `
      <tr>
        <td style="font-weight:600">${label(a)}</td>
...
function pcGetPlanPersonalData() {
  ...
  if(!tb) return [];
  return [...tb.rows].map(tr => {
    const employee = tr.querySelector('[data-field="employee"]')?.value || '';
    const area = tr.querySelector('[data-field="area"]')?.value || '';
    const days = {};
    tr.querySelectorAll('[data-day]').forEach(inp => {
...
function pcApplyImportedPersonal(records) {
  ...
  records.forEach(rec => {
    ...
    let existingTr = [...tb.rows].find(tr => tr.querySelector('[data-field="employee"]')?.value === rec.employee);
    if(!existingTr) {
      pcAddPersonaRow({employee: rec.employee, area: rec.area||'', days: daysInRange});
      personasNuevas++;
    } else {
      if(rec.area) {
        const areaSel = existingTr.querySelector('[data-field="area"]');
        if(areaSel && areaSel.value !== rec.area.toUpperCase()) {
          const opt = [...areaSel.options].find(o=>o.value===rec.area.toUpperCase());
          if(opt) areaSel.value = rec.area.toUpperCase();
        }
      }
      Object.entries(daysInRange).forEach(([date,hours]) => {
        const inp = existingTr.querySelector(`[data-day="${date}"]`);
        if(inp) inp.value = hours;
      });
      pcPersonaRowRecalc(existingTr.id);
    }
  });
