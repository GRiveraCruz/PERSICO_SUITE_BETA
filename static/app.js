...
const CAP_INT_BG = 'rgba(66,99,148,.20)';
const CAP_INT_COLOR = '#2c3e50';
...
function capLegendRenderRows(){
  ...
  while(padded.length<10) padded.push({codigo:'',descripcion:'',esInterno:false});
  tb.innerHTML=padded.map(row=>`<tr>
    ...
    <td style="padding:4px 8px!important"><input type="checkbox" ${row.esInterno?'checked':''} title="Trabajo interno (en instalaciones de la empresa)" style="width:18px;height:18px;cursor:pointer" onchange="capLegendSave()"></td>
  </tr>`).join('');
  if(!permCanCreate('ops-capacidad')){
...
async function capLegendSave(){
  ...
  const rows=Array.from(tb.children).map(tr=>{
    ...
    return {codigo:inps[0].value.trim().toUpperCase(), descripcion:inps[1].value.trim(), esInterno:inps[2].checked};
  });
  try{
...
function capLegendMap(){
  const map={};
  capLegendRows.forEach(r=>{ if(r.codigo && r.esInterno) map[r.codigo.trim().toUpperCase()]=true; });
  return map;
}
...
function capApplyColors(){
  ...
  document.querySelectorAll('#cap-wrap .cap-input').forEach(inp=>{
    ...
    const esInterno=!!map[v];
    inp.style.color=esInterno?CAP_INT_COLOR:'';
    inp.style.backgroundColor=esInterno?CAP_INT_BG:'';
  });
}

async function capSaveCell(tid,fecha,value,el){
  const codigo=value.trim().toUpperCase();
  if(el){ const esInterno=!!capLegendMap()[codigo]; el.style.color=esInterno?CAP_INT_COLOR:''; el.style.backgroundColor=esInterno?CAP_INT_BG:''; }
  try{
