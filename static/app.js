...
async function ctrlConfirmExport(){
  ...
  try{
    ...
    let msg=`✅ ${r.added} registro(s) exportados a Mano de Obra`;
...
function whRender(){
  ...
  tb.innerHTML=display.map((r,i)=>{
    ...
    return`<tr style="${i%2===0?'':'background:rgba(0,0,0,.02)'}">
      ...
      <td style="text-align:right;font-family:'DM Mono',monospace;font-size:12px;color:${isFixed?'var(--blue)':(hasRate?'var(--green)':'var(--muted)')}" title="${isFixed?'Costo fijo desde Sueldos y Salarios: $'+rate.toFixed(2)+'/hr':(hasRate?'$'+rate.toFixed(2)+'/hr':'Sin tarifa en Hourly Rates')}">
