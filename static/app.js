...
async function recBuscarPO(val) {
  ...
  try {
    const d = await fetch(`/api/gpo/${encodeURIComponent(q)}/lookup`).then(r=>r.json());
    if(d.error) {
      toast(d.error, 'er');
      document.getElementById('rec-step-proveedor').style.display = 'none';
      document.getElementById('rec-step-po-content').style.display = 'none';
...
// ── Paso: mostrar items de la OC (filtrables) ────────────
    ...
    tb.innerHTML = recDirectaItems.map((it, i) => `
      <tr>
        ...
        <td style="text-align:right"><input type="number" min="0" step="1" value="${it.quantity_received}"
          oninput="recManualUpdate(${i},'quantity_received',this.value)"
      ...
      </tr>`).join('') || `<tr><td colspan="8" style="text-align:center;color:var(--muted);padding:16px">Agrega items con el botón "+ Agregar Item"</td></tr>`;
    return;
  }

  const filter = (document.getElementById('rec-items-filter')?.value || '').toLowerCase();
  const items = (recCurrentPO?.items || []);
  const completa = recTipoRecepcion === 'completa';
  tb.innerHTML = items.map((it, i) => {
    if(filter && !((it.part_number||'').toLowerCase().includes(filter) || (it.description||'').toLowerCase().includes(filter))) return '';
    const qOrd = parseFloat(it.quantity) || 1;
    const qDel = parseFloat(it.quantity_delivered || 0);
    const qPend = Math.max(0, qOrd - qDel);
    const defaultQty = completa ? qPend : 0;
    return `<tr data-idx="${i}">
      <td><input type="checkbox" class="rec-item-chk" data-idx="${i}" ${defaultQty>0?'checked':''} onchange="recToggleRow(${i})" style="width:15px;height:15px"></td>
      <td style="font-family:'DM Mono',monospace">${esc(it.part_number||'—')}</td>
      <td style="color:var(--muted2);max-width:220px;overflow:hidden;text-overflow:ellipsis">${esc(it.description||'—')}</td>
      <td style="font-family:'DM Mono',monospace;color:var(--gold)">${esc(it.job||'—')}</td>
      <td style="text-align:right">${qOrd}</td>
      <td style="text-align:right;color:${qDel>=qOrd?'var(--green)':'var(--muted)'}">${qDel}</td>
      <td style="text-align:right">
        <input type="number" min="0" max="${qPend}" step="1" value="${defaultQty}"
          id="rec-qty-${i}" ${completa?'readonly':''}
          style="width:70px;background:var(--inp);border:1px solid var(--border);border-radius:4px;color:var(--gold);padding:4px 7px;font-size:12px;text-align:right;font-weight:700">
      </td>
      <td style="text-align:right;font-family:'DM Mono',monospace">$${Number(it.unit_price||0).toFixed(2)}</td>
    </tr>`;
  }).join('');
...
function recToggleRow(i) {
  const chk = document.querySelector(`.rec-item-chk[data-idx="${i}"]`);
  const qtyEl = document.getElementById(`rec-qty-${i}`);
  if(qtyEl && !chk.checked) qtyEl.value = 0;
}
...
function recRecolectarItems() {
  ...
  items.forEach((it, i) => {
    const chk = document.querySelector(`.rec-item-chk[data-idx="${i}"]`);
    const qtyEl = document.getElementById(`rec-qty-${i}`);
    if(!chk || !chk.checked || !qtyEl) return;
    const qty = parseFloat(qtyEl.value || 0);
    if(qty <= 0) return;
    out.push({
      part_number: it.part_number, description: it.description, brand: it.brand||'',
      job: it.job||'', quantity_received: qty, unit_cost: parseFloat(it.unit_price||0)
    });
  });
  return out;
}
...
async function recRegistrar() {
  ...
  const items = recRecolectarItems();
  if(!items.length) { toast('No hay items seleccionados', 'er'); return; }

  const body = {
    po_number: recCompraDirecta ? '' : (recCurrentPO?.po_number || ''),
    compra_directa: recCompraDirecta,
    tipo_recepcion: recTipoRecepcion,
    supplier: recCompraDirecta ? (recDirectaSupplier || {}) : undefined,
    factura, items,
  };
