...
async function recRegistrar() {
  ...
  if(recCompraDirecta) {
    ...
    body = { compra_directa: true, supplier: recDirectaSupplier || {}, factura, items };
  } else {
...
function pagoRenderProvTabla() {
  ...
  tb.innerHTML = pagoProvPendientes.map((c,i) => `
    ...
    </tr>`).join('') || `<tr><td colspan="7" style="text-align:center;color:var(--muted);padding:16px">No hay CPP pendientes de pago${val?' para ese proveedor':''}</td></tr>`;
  pagoUpdateBtnState();
}
