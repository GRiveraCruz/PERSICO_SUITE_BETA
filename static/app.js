...
document.addEventListener('keydown',e=>{
  if(e.key==='Escape'){
    const mods=['mo-jnew','mo-jimp','mo-rnew','mo-rimp','mo-rcopy','mo-qnew','mo-qimp','mo-pt-new','mo-pt-confirm','mo-sv-new','mo-stk-imp','mo-stk-ing','mo-reassign','mo-prov-new','mo-prov-imp','mo-cat-new','mo-cat-imp','mo-cpo-new','mo-cpo-imp','mo-po-imp','mo-wh-imp','mo-ivp-imp','mo-fx-imp','mo-refuse','mo-award','mo-tnew','mo-timp','mo-vac-add','mo-asis-link','mo-pm-new','mo-sal-unlock','mo-ctrl-export','mo-sal-import-excel','mo-sal-export-excel','mo-isr-import','mo-np-periodo','mo-np-generar','mo-np-recibo','mo-cpc-new','mo-os','mo-ta','mo-cap-os-detalle','mo-emisor-fiscal'];
...
async function loadCPC(){
  const btnEmisor=document.getElementById('btn-cpc-emisor');
  if(btnEmisor) btnEmisor.style.display=(USER_PERMS&&USER_PERMS.is_admin)?'':'none';
...
function cpcOpenNew(){
  ...
  cpcToggleCpoMode();
  document.querySelector('#mo-cpc-new .cl-toggle').style.display='';
  cpcSetModo('cargar');
  cpcGenConceptos=[];
  document.getElementById('cpc-gen-rfc').value='';
  document.getElementById('cpc-gen-razon').value='';
  document.getElementById('cpc-gen-email').value='';
  document.getElementById('cpc-gen-cp').value='';
  document.getElementById('cpc-gen-persona-manual').value='';
  document.getElementById('cpc-gen-error').textContent='';
  document.getElementById('cpc-gen-receptor-status').textContent='';
  cpcGenRenderConceptos();
...
function cpcSetModo(modo){
  const esGenerar=modo==='generar';
  document.getElementById('cpc-modo-cargar').style.display=esGenerar?'none':'';
  document.getElementById('cpc-modo-generar').style.display=esGenerar?'':'none';
  document.getElementById('cpc-modo-cargar-btn').classList.toggle('on',!esGenerar);
  document.getElementById('cpc-modo-generar-btn').classList.toggle('on',esGenerar);
  document.getElementById('cpc-btn-guardar').style.display=esGenerar?'none':'';
}
...
function cpcOpenEdit(id){
  ...
  cpcSetModo('cargar');
  document.querySelector('#mo-cpc-new .cl-toggle').style.display='none';
  document.getElementById('cpc-btn-guardar').style.display='';
...
// ── Generar y Timbrar con FISCAL API ──
let cpcGenConceptos=[];

function cpcGenAddConcepto(){
  cpcGenConceptos.push({descripcion:'',clave_prod_sat:'',clave_unidad:'H87',cantidad:1,valor_unitario:0,objeto_imp:'02',iva_pct:16});
  cpcGenRenderConceptos();
}
function cpcGenRemoveConcepto(i){
  cpcGenConceptos.splice(i,1);
  cpcGenRenderConceptos();
}
function cpcGenUpdateConcepto(i,field,val){
  cpcGenConceptos[i][field]=(field==='descripcion'||field==='clave_prod_sat'||field==='clave_unidad'||field==='objeto_imp')?val:parseFloat(val)||0;
  cpcGenRenderConceptos(true);
}
function cpcGenRenderConceptos(keepFocus){
  const box=document.getElementById('cpc-gen-conceptos-list');
  if(!cpcGenConceptos.length){ box.innerHTML='<div style="font-size:11px;color:var(--muted)">Sin conceptos — agrega al menos uno.</div>'; cpcGenUpdateTotales(); return; }
  box.innerHTML=cpcGenConceptos.map((c,i)=>{
    const importe=(c.cantidad||0)*(c.valor_unitario||0);
    return `<div style="background:var(--sb);border-radius:var(--r);padding:10px;display:flex;flex-direction:column;gap:6px">
      <div style="display:flex;gap:6px">
        <input type="text" placeholder="Descripción" value="${esc(c.descripcion)}" oninput="cpcGenUpdateConcepto(${i},'descripcion',this.value)" style="flex:2;padding:6px 8px;border:1px solid var(--border);border-radius:4px;background:var(--inp);color:var(--text);font-size:12px">
        <input type="text" placeholder="Clave SAT" value="${esc(c.clave_prod_sat)}" oninput="cpcGenUpdateConcepto(${i},'clave_prod_sat',this.value)" style="flex:1;padding:6px 8px;border:1px solid var(--border);border-radius:4px;background:var(--inp);color:var(--text);font-size:12px;font-family:'DM Mono',monospace">
        <button class="fi-del" onclick="cpcGenRemoveConcepto(${i})">🗑</button>
      </div>
      <div style="display:flex;gap:6px;align-items:center">
        <input type="number" placeholder="Cant." value="${c.cantidad}" oninput="cpcGenUpdateConcepto(${i},'cantidad',this.value)" style="width:70px;padding:6px 8px;border:1px solid var(--border);border-radius:4px;background:var(--inp);color:var(--text);font-size:12px">
        <input type="number" placeholder="Valor Unitario" value="${c.valor_unitario}" oninput="cpcGenUpdateConcepto(${i},'valor_unitario',this.value)" style="width:110px;padding:6px 8px;border:1px solid var(--border);border-radius:4px;background:var(--inp);color:var(--text);font-size:12px">
        <input type="text" placeholder="Clave Unidad" value="${esc(c.clave_unidad)}" oninput="cpcGenUpdateConcepto(${i},'clave_unidad',this.value)" style="width:90px;padding:6px 8px;border:1px solid var(--border);border-radius:4px;background:var(--inp);color:var(--text);font-size:12px;font-family:'DM Mono',monospace">
        <select onchange="cpcGenUpdateConcepto(${i},'iva_pct',this.value)" style="padding:6px 8px;border:1px solid var(--border);border-radius:4px;background:var(--inp);color:var(--text);font-size:12px">
          <option value="16" ${c.iva_pct==16?'selected':''}>IVA 16%</option>
          <option value="0" ${c.iva_pct==0?'selected':''}>IVA 0%</option>
        </select>
        <span style="margin-left:auto;font-family:'DM Mono',monospace;font-size:12px;color:var(--gold);white-space:nowrap">$${importe.toLocaleString('en-US',{minimumFractionDigits:2})}</span>
      </div>
    </div>`;
  }).join('');
  cpcGenUpdateTotales();
}
function cpcGenUpdateTotales(){
  let subtotal=0, iva=0;
  cpcGenConceptos.forEach(c=>{
    const importe=(c.cantidad||0)*(c.valor_unitario||0);
    subtotal+=importe; iva+=importe*(c.iva_pct||0)/100;
  });
  document.getElementById('cpc-subtotal').value=subtotal.toFixed(2);
  document.getElementById('cpc-iva').value=iva.toFixed(2);
  cpcRecalcTotal();
}

async function cpcGenerarYTimbrar(btn){
  const errBox=document.getElementById('cpc-gen-error');
  errBox.textContent='';
  const rfc=document.getElementById('cpc-gen-rfc').value.trim().toUpperCase();
  const razon=document.getElementById('cpc-gen-razon').value.trim();
  const cp=document.getElementById('cpc-gen-cp').value.trim();
  const job=document.getElementById('cpc-job').value.trim();
  const fechaFactura=document.getElementById('cpc-fecha-factura').value;
  if(!rfc||!razon||!cp){ errBox.textContent='Completa RFC, Razón Social y Código Postal del receptor'; return; }
  if(!job){ errBox.textContent='Completa el Job (abajo, en la sección compartida)'; return; }
  if(!fechaFactura){ errBox.textContent='Completa la Fecha de Factura'; return; }
  if(!cpcGenConceptos.length){ errBox.textContent='Agrega al menos un concepto'; return; }
  for(const c of cpcGenConceptos){
    if(!c.descripcion||!c.clave_prod_sat||!c.valor_unitario){ errBox.textContent='Completa Descripción, Clave SAT y Valor Unitario de todos los conceptos'; return; }
  }

  // Autocompletar cliente con la razón social del receptor si no se capturó
  if(!document.getElementById('cpc-cliente').value.trim()) document.getElementById('cpc-cliente').value=razon;
  cpcGenUpdateTotales();

  const btnEl=btn; btnEl.disabled=true; btnEl.textContent='Generando…';
  try{
    // 1) Crear el registro base de la CPC (como cualquier factura manual)
    const payload={
      year: parseInt(fechaFactura.slice(0,4)),
      job, cpo_id: document.getElementById('cpc-usar-cpo').checked ? document.getElementById('cpc-cpo-select').value : null,
      customer: document.getElementById('cpc-cliente').value.trim(),
      fecha_factura: fechaFactura, fecha_vencimiento: document.getElementById('cpc-fecha-venc').value,
      subtotal: document.getElementById('cpc-subtotal').value, iva: document.getElementById('cpc-iva').value,
      isr_retenido: document.getElementById('cpc-isr-ret').value, iva_retenido: document.getElementById('cpc-iva-ret').value,
      total: document.getElementById('cpc-total-input').value,
    ...
    const created=await apiCall('POST','/cpc',payload);
    if(created.error){ errBox.textContent=created.error; return; }
    const newId=created.record.id;

    // 2) Generar y timbrar con FISCAL API
    const body={
      year: payload.year,
      receptor:{
        rfc, razon_social:razon, email:document.getElementById('cpc-gen-email').value.trim(),
        codigo_postal:cp, regimen_fiscal:document.getElementById('cpc-gen-regimen').value,
        uso_cfdi:document.getElementById('cpc-gen-uso').value,
        persona_id_manual:document.getElementById('cpc-gen-persona-manual').value.trim(),
      },
      conceptos:cpcGenConceptos,
      forma_pago:document.getElementById('cpc-gen-forma-pago').value,
      metodo_pago:document.getElementById('cpc-gen-metodo-pago').value,
    };
    const r=await apiCall('POST','/cpc/'+newId+'/timbrar-fiscalapi',body);
    if(r.error){
      let msg=r.error;
      if(r.sugerencia) msg+=' — '+r.sugerencia;
      errBox.innerHTML=`⚠ La factura se creó (${newId}) pero no se pudo timbrar: ${esc(msg)}<br><br>Puedes editarla y reintentar el timbrado, o completar el ID manual que te haya pedido.`;
      await loadCPC();
      return;
    }
    closeMo('mo-cpc-new');
    toast(`Factura ${newId} timbrada ✓ UUID: ${r.record.fiscalapi_uuid||'—'}`,'ok',8000);
    await loadCPC();
  }catch(e){ errBox.textContent='Error: '+e; }
  finally{ btnEl.disabled=false; btnEl.textContent='🖊 Generar y Timbrar Factura'; }
}

// ── Datos Fiscales del Emisor ──
async function cpcOpenEmisorFiscal(){
  try{
    const d=await(await fetch('/api/emisor-fiscal')).json();
    document.getElementById('ef-rfc').value=d.rfc||'';
    document.getElementById('ef-razon').value=d.razon_social||'';
    document.getElementById('ef-email').value=d.email||'';
    document.getElementById('ef-cp').value=d.codigo_postal||'';
    if(d.regimen_fiscal) document.getElementById('ef-regimen').value=d.regimen_fiscal;
    document.getElementById('ef-fiscalapi-id').value=d.fiscalapi_persona_id||'';
    document.getElementById('ef-error').textContent='';
  }catch(e){}
  document.getElementById('mo-emisor-fiscal').classList.add('on');
}
async function efSave(){
  const errBox=document.getElementById('ef-error');
  const payload={
    rfc:document.getElementById('ef-rfc').value.trim(),
    razon_social:document.getElementById('ef-razon').value.trim(),
    email:document.getElementById('ef-email').value.trim(),
    codigo_postal:document.getElementById('ef-cp').value.trim(),
    regimen_fiscal:document.getElementById('ef-regimen').value,
    fiscalapi_persona_id:document.getElementById('ef-fiscalapi-id').value.trim(),
  };
  if(!payload.rfc||!payload.razon_social||!payload.codigo_postal){ errBox.textContent='Completa RFC, Razón Social y Código Postal'; return; }
  try{
    const r=await apiCall('PUT','/emisor-fiscal',payload);
    if(r.error){ errBox.textContent=r.error; return; }
    closeMo('mo-emisor-fiscal');
    toast('Datos fiscales del emisor guardados ✓','ok');
  }catch(e){ errBox.textContent='Error: '+e; }
}
