...
document.addEventListener('DOMContentLoaded',async ()=>{ await loadAreas(); await loadPerfiles(); await loadPersonal(); await loadCapacidad(); });
...
function capRenderAreaTable(groupName, workers, days){
  ...
  const rows=workers.map(w=>{
    const cells=days.map(d=>{
      ...
      return `<td class="${d.isWeekend?'cap-weekend':''}${d.isWeekStart?' cap-sep':''}"><input class="cap-input" value="${esc(val)}" onchange="capSaveCell('${w.tid}','${d.str}',this.value)" onpaste="capOnPaste(event)"></td>`;
    }).join('');
...
async function capSaveCell(tid,fecha,value){
