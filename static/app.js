...
function recRenderCFDI(cfdi) {
  ...
  if(cfdi._ya_procesado) {
    ...
    // Comparar el total del CFDI contra el total de los items que se van a fiscalizar
    const totalRec = recCompraDirecta
      ? recRecolectarItems().reduce((s,it)=>s+(it.quantity_received*it.unit_cost),0)
      : (recCurrentPO?.ingresos||[]).reduce((s,ing,i)=>{
          const chk = document.querySelector(`.rec-item-chk[data-idx="${i}"]`);
