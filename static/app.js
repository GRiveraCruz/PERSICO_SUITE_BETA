...
function pmRender(){
  ...
  tb.innerHTML=rows.map(({pm,nombre})=>{
    ...
    return `<tr class="${pm.id===pmCurrentId?'sel':''}" onclick="pmOpen('${pm.id}')">
      ...
      <td style="font-family:'DM Mono',monospace;font-size:12px">${pm.fecha_inicio}</td>
      <td style="font-family:'DM Mono',monospace;font-size:12px">${pm.fecha_fin}</td>
      <td style="text-align:center;font-family:'DM Mono',monospace">${pm.dias}</td>
...
function pmOpenNew(){
  ...
  document.getElementById('pmn-tipo').value='Con goce de sueldo';
...
async function pmCreate(){
  ...
  const fecha_inicio=document.getElementById('pmn-inicio').value;
  const fecha_fin=document.getElementById('pmn-fin').value;
  const motivo=document.getElementById('pmn-motivo').value.trim();
  if(!tid||!tipo||!fecha_inicio||!fecha_fin){ toast('Completa los campos obligatorios','er'); return; }
  const btn=document.getElementById('btn-pmn-create'); btn.disabled=true; btn.textContent='Solicitando…';
  try{
    const r=await apiCall('POST','/permisos',{tid,tipo,fecha_inicio,fecha_fin,motivo});
    if(r.error){ toast(r.error,'er'); return; }
    closeMo('mo-pm-new');
...
function pmOpen(id){
  ...
  document.getElementById('pme-dias').textContent=pm.dias+(pm.dias===1?' día':' días');
  ...
  ['pme-tipo','pme-inicio','pme-fin','pme-motivo'].forEach(id=>document.getElementById(id).disabled=soloLectura);
...
async function pmSave(){
  ...
  const payload={
    ...
    fecha_inicio:document.getElementById('pme-inicio').value,
    fecha_fin:document.getElementById('pme-fin').value,
