...
function clCalcular(){
  ...
  const rows=[
    {grp:'Sueldo Base'},
    {label:'Sueldo Mensual', val:mensual},
  ...
  const bodyRows=rows.map(r=>{
    ...
    const cls=r.tot?'tot':(r.sub?'sub':'');
    return `<tr class="${cls}">
      <td>${esc(r.label)}</td>
      <td class="n">${clMoney(r.val)}</td>
      <td><div class="sal-bar-wrap"><div class="sal-bar"><i style="width:${Math.min(100,pct).toFixed(1)}%"></i></div><span class="sal-bar-pct">${pct.toFixed(1)}%</span></div></td>
    </tr>`;
  }).join('');

  results.innerHTML=`
    <div class="sal-kpis">
      <div class="sal-kpi"><div class="l">Sueldo Base Mensual</div><div class="v">${clMoney(mensual)}</div></div>
      <div class="sal-kpi"><div class="l">Prestaciones Mensuales</div><div class="v">${clMoney(prestacionesMes)}</div></div>
      <div class="sal-kpi red"><div class="l">Cargas Sociales Mensuales</div><div class="v">${clMoney(cargasSocialesMes)}</div></div>
      <div class="sal-kpi red"><div class="l">Imp. Sobre Nómina Mensual</div><div class="v">${clMoney(isnMes)}</div></div>
    </div>
    <div class="sal-kpis" style="grid-template-columns:1fr 1fr">
      <div class="sal-kpi hl"><div class="l">Costo Mensual Total para la Empresa</div><div class="v">${clMoney(costoMensualTotal)}</div></div>
      <div class="sal-kpi hl"><div class="l">Costo Anual Total para la Empresa</div><div class="v">${clMoney(costoAnualTotal)}</div></div>
    </div>
    <div class="sal-card" style="margin-top:4px">
    ...
    <div class="sal-hint" style="margin-top:6px">Estimación con fines de planeación. Verifica UMA, prima de riesgo, tasa de Cesantía y Vejez e Impuesto Sobre Nómina vigentes antes de usar esta cifra para fines fiscales u oficiales.</div>
  `;
}
