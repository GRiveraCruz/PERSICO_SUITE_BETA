...
function pcRenderPersonalTable() {
  ...
  const fxBody = document.getElementById('pc-personal-fixed-body');
  ...
  const rowsSource = fxBody.rows.length ? pcGetPlanPersonalData() : (pcPersonalSaved||[]);
  ...
  scHead.innerHTML = days.map(dt =>
    `<th style="width:44px;text-align:center;font-size:9px;color:${[0,6].includes(dt.getDay())?'var(--muted)':'var(--muted2)'}">${dt.toLocaleDateString('es-MX',{day:'2-digit',month:'2-digit'})}</th>`
  ).join('');
...
function pcAddPersonaRow(data={}) {
  ...
  fxTr.id = `fx-${rowId}`;
  fxTr.style.height = '36px';
  fxTr.innerHTML = `
    <td><select data-field="employee" onchange="pcPersonaRowRecalc('${rowId}')" style="width:100%;background:var(--inp);border:1px solid var(--border);border-radius:4px;color:var(--text);padding:4px 6px;font-size:11px">
      ${options.join('')}
    </select></td>
    <td><select data-field="area" onchange="pcUpdatePersonalTotals()" style="width:100%;background:var(--inp);border:1px solid var(--border);border-radius:4px;color:var(--amber);padding:4px 6px;font-size:11px">
      ${areaOptions.join('')}
    </select></td>
    <td style="text-align:right;font-family:'DM Mono',monospace" id="${rowId}-rate">—</td>
    <td style="text-align:right;font-family:'DM Mono',monospace;font-weight:700" id="${rowId}-hours">0</td>
    <td style="text-align:right;font-family:'DM Mono',monospace;font-weight:700;color:var(--gold)" id="${rowId}-amount">$0.00</td>
    <td style="text-align:center"><button onclick="pcRemovePersonaRow('${rowId}')" style="background:none;border:none;color:var(--muted);cursor:pointer;font-size:13px">✕</button></td>
  `;
  fxBody.appendChild(fxTr);

  const daysVal = data.days || {};
  const scTr = document.createElement('tr');
  scTr.id = `sc-${rowId}`;
  scTr.style.height = '36px';
  scTr.innerHTML = pcPersonalDayKeys.map(key =>
    `<td style="padding:2px"><input data-day="${key}" type="number" min="0" step="0.5" value="${daysVal[key]||''}" oninput="pcPersonaRowRecalc('${rowId}')" style="width:40px;background:var(--inp);border:1px solid var(--border);border-radius:3px;color:var(--text);padding:3px;font-size:10px;text-align:center"></td>`
  ).join('');
  scBody.appendChild(scTr);
...
function pcPersonaRowRecalc(rowId) {
  const fxTr = document.getElementById(`fx-${rowId}`);
  const scTr = document.getElementById(`sc-${rowId}`);
  if(!fxTr || !scTr) return;
  const emp  = fxTr.querySelector('[data-field="employee"]')?.value || '';
  const rate = Object.prototype.hasOwnProperty.call(pcPersonalRatesMap, emp) ? pcPersonalRatesMap[emp] : null;
  const hours = [...scTr.querySelectorAll('[data-day]')].reduce((s,inp)=>s+(parseFloat(inp.value)||0),0);
...
function pcUpdatePersonalTotals() {
  ...
  [...fxBody.rows].forEach(fxTr => {
    const rowId = fxTr.id.replace(/^fx-/, '');
    const scTr = document.getElementById(`sc-${rowId}`);
    const emp = fxTr.querySelector('[data-field="employee"]')?.value || '';
    const area = (fxTr.querySelector('[data-field="area"]')?.value || '').toUpperCase();
    const rate = pcPersonalRatesMap[emp] ?? 0;
    const hours = scTr ? [...scTr.querySelectorAll('[data-day]')].reduce((s,inp)=>s+(parseFloat(inp.value)||0),0) : 0;
...
function pcGetPlanPersonalData() {
  const fxBody = document.getElementById('pc-personal-fixed-body');
  if(!fxBody) return [];
  return [...fxBody.rows].map(fxTr => {
    const rowId = fxTr.id.replace(/^fx-/, '');
    const scTr = document.getElementById(`sc-${rowId}`);
    const employee = fxTr.querySelector('[data-field="employee"]')?.value || '';
    const area = fxTr.querySelector('[data-field="area"]')?.value || '';
    const days = {};
    if(scTr) scTr.querySelectorAll('[data-day]').forEach(inp => {
...
function pcApplyImportedPersonal(records) {
  ...
  records.forEach(rec => {
    ...
    let existingFxTr = [...fxBody.rows].find(tr => tr.querySelector('[data-field="employee"]')?.value === rec.employee);
    if(!existingFxTr) {
      pcAddPersonaRow({employee: rec.employee, area: rec.area||'', days: daysInRange});
      personasNuevas++;
    } else {
      const rowId = existingFxTr.id.replace(/^fx-/, '');
      if(rec.area) {
        const areaSel = existingFxTr.querySelector('[data-field="area"]');
        if(areaSel) {
      ...
      const scTr = document.getElementById(`sc-${rowId}`);
      if(scTr) Object.entries(daysInRange).forEach(([date,hours]) => {
        const inp = scTr.querySelector(`[data-day="${date}"]`);
