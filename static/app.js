...
function pcSwitchTab(tab) {
  pcCurrentTab = tab;
  const tabs = {presupuesto:'pc-tab-presupuesto', timing:'pc-tab-timing', abiertos:'pc-tab-abiertos', cambios:'pc-tab-cambios'};
  Object.entries(tabs).forEach(([k,id])=>{
    const el = document.getElementById(id); if(!el) return;
    el.style.background = k===tab ? 'var(--red)' : 'rgba(0,0,0,.055)';
    el.style.color      = k===tab ? '#fff' : 'var(--muted)';
  });
  ['presupuesto','timing','abiertos','cambios'].forEach(k=>{
...
function pcRenderJobs(jobDetails, savedRows) {
  ...
  document.getElementById('pc-jobs-body').innerHTML = jobDetails.map((j,idx) => {
    ...
    const est = {
      ...
      materiales:    saved.est_materiales ?? '',
    ...
    <div class="pc-job-card" id="pc-row-${idx}" style="background:var(--card);border:1px solid var(--border);border-radius:10px;padding:16px 20px">
      ...
      <div style="border-top:1px dashed var(--border);padding-top:12px">
        ...
        <div style="display:grid;grid-template-columns:repeat(5,1fr);gap:10px">
          ...
          <div><div style="font-size:10px;color:var(--muted);margin-bottom:3px">Materiales</div><input data-field="est_materiales" type="number" min="0" step="0.01" value="${est.materiales}" placeholder="$0.00" oninput="pcCalc(${idx})" style="${estInp}"></div>
...
function pcCalc(idx) {
  ...
  const estSum    = f('est_ing_mecanica') + f('est_ing_electrica') + f('est_major_items') + f('est_materiales') + f('est_ensamble');
  const fmt = v => '$'+Number(v).toLocaleString('en-US',{minimumFractionDigits:2});

  const montoMarkup = revenue * markupPct / 100;
...
async function pcSave() {
  ...
  const jobsData = pcJobRows.map((j,idx) => {
    ...
    const estMat     = fn('est_materiales');
    const estEns     = fn('est_ensamble');
    const sumaEst    = estIngMec + estIngElec + estMajor + estMat + estEns;
    // Compatibilidad con Job Cost Report: Compras ≈ Materiales + Major Items; M.O. ≈ Ingeniería + Ensamble
    const targetCompras = estMat + estMajor;
  ...
  try {
    const d = await fetch('/api/projconfig',{
      ...
      body: JSON.stringify({ptsv: pcCurrentPTSV, jobs: jobsData, timing: timingData,
        puntos_abiertos: puntosData, control_cambios: cambiosData})
    }).then(r=>r.json());
    if(d.error){toast(d.error,'er');return;}
