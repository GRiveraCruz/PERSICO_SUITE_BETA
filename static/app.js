...
function clCalcular(){
  ...
  const isrMes=parseFloat(document.getElementById('cl-isr-mensual').value)||0;
  ...
  // ── IMSS Obrero (cuotas a cargo del trabajador, tasas fijas de ley) ──
  const obExcedenteMes    = uma>0 ? 0.0040*excedenteBase*diasMes : 0;
  const obPrestDineroMes  = 0.0025*sbc*diasMes;
  const obGastosMedMes    = 0.00375*sbc*diasMes;
  const obInvalidezMes    = 0.00625*sbc*diasMes;
  const obCyvMes          = 0.01125*sbc*diasMes;
  const imssObreroMes = obExcedenteMes+obPrestDineroMes+obGastosMedMes+obInvalidezMes+obCyvMes;

  const sueldoNetoMes = mensual-imssObreroMes-isrMes;
  ...
  // ── Deducciones al trabajador (tabla aparte) ──
  const netoRows=[
    {label:'Sueldo Mensual (bruto)', val:mensual},
    {label:'IMSS Obrero (Excedente + Dinero + G. Médicos + Invalidez + Cesantía y Vejez)', val:-imssObreroMes},
    {label:'ISR Retenido (manual)', val:-isrMes},
    {label:'Sueldo Neto Mensual (a pagar al trabajador)', val:sueldoNetoMes, neto:true},
  ];
  const netoBodyRows=netoRows.map(r=>`<tr class="${r.neto?'neto':''}">
      <td>${esc(r.label)}</td>
      <td class="n">${r.val<0?'– '+clMoney(Math.abs(r.val)):clMoney(r.val)}</td>
      <td></td>
    </tr>`).join('');

  clLastData={diario,mensual,diasAguinaldo,diasVac,primaVacPct,uma,primaRiesgoPct,cvPct,aplicarTope,isnPct,isrMes,
    factorIntegracion,sdi,sbc,topeAplicado,prestacionesMes,imssTotalMes,infonavitMes,cargasSocialesMes,isnMes,
    costoMensualTotal,costoAnualTotal,imssObreroMes,sueldoNetoMes,rows,netoRows};
  ...
  results.innerHTML=`
    ...
    <div class="cl-kpis" style="grid-template-columns:1fr">
      <div class="cl-kpi" style="background:rgba(39,174,96,.1);border-color:rgba(39,174,96,.4)"><div class="l" style="color:#1f8a4c">Sueldo Neto Mensual (a pagar al trabajador)</div><div class="v" style="color:#1f8a4c;font-size:22px">${clMoney(sueldoNetoMes)}</div></div>
    </div>
    ...
    <div class="cl-card">
      <h3>👤 Deducciones al Trabajador → Sueldo Neto</h3>
      <table class="cl-tbl">
        <thead><tr><th>Concepto</th><th style="text-align:right">Mensual</th><th></th></tr></thead>
        <tbody>${netoBodyRows}</tbody>
      </table>
    </div>
    <div class="cl-hint" style="margin-top:6px">Estimación con fines de planeación. Verifica UMA, prima de riesgo, tasa de Cesantía y Vejez, Impuesto Sobre Nómina e ISR retenido vigentes antes de usar esta cifra para fines fiscales u oficiales.</div>
  `;
}
...
function clExportPDF(){
  ...
  const netoHtml=d.netoRows.map(r=>`<tr class="${r.neto?'neto':''}"><td>${esc(r.label)}</td><td class="n">${r.val<0?'– '+clMoney(Math.abs(r.val)):clMoney(r.val)}</td></tr>`).join('');
  ...
  const html=`<!DOCTYPE html><html><head><meta charset="utf-8"><title>Costo Laboral</title><style>
    *{box-sizing:border-box}
    ...
    .card.neto{background:#eafaf0;border-color:#1f8a4c}
    .card.neto .l{color:#1f8a4c}
    .card.neto .v{color:#1f8a4c}
    ...
    tr.neto td{font-weight:800;font-size:13px;background:#eafaf0;color:#1f8a4c;border-top:2px solid #1f8a4c}
    ...
    <div class="cards">
      ...
      <div class="card neto"><div class="l">Sueldo Neto Mensual</div><div class="v">${clMoney(d.sueldoNetoMes)}</div></div>
    </div>
    ...
    <h2>Deducciones al Trabajador → Sueldo Neto</h2>
    <table><thead><tr><th>Concepto</th><th style="text-align:right">Mensual (MXN)</th></tr></thead><tbody>${netoHtml}</tbody></table>
    <div class="note">Estimación con fines de planeación interna. Verifica UMA, prima de riesgo de trabajo, tasa de Cesantía y Vejez, Impuesto Sobre Nómina e ISR retenido vigentes antes de usar esta cifra para fines fiscales u oficiales.</div>
