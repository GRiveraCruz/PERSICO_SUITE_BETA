...
# ══════════════════════════════════════════════════════════════════
#  FISCAL API — Timbrado real de CFDI (PAC elegido por Persico)
#  Contrato confirmado con su documentación (docs.fiscalapi.com):
#    POST /api/v4/invoices  con headers X-TENANT-KEY / X-API-KEY / X-TIME-ZONE
#    body: {..., issuer:{id}, recipient:{id}, items:[{id,quantity,discount}]}
#  Los endpoints para crear "Persona" (emisor/receptor) y "Producto" (concepto)
#  vía API NO están confirmados con un ejemplo real todavía — se implementan
#  con la mejor suposición posible (nombres de campo estilo camelCase, igual
#  que en /invoices) y, si fallan, se ofrece un flujo manual: crear el recurso
#  en el dashboard de FISCAL API y pegar aquí su ID.
# ══════════════════════════════════════════════════════════════════
FISCALAPI_BASE_URL = _os.environ.get("FISCALAPI_BASE_URL", "https://test.fiscalapi.com")
EMISOR_FISCAL_FILE = _os.path.join(_DATA, "emisor_fiscal.json")
FISCALAPI_PERSONAS_CACHE_FILE = _os.path.join(_DATA, "fiscalapi_personas.json")

def _load_emisor_fiscal():
    p = Path(EMISOR_FISCAL_FILE)
    if p.exists():
        try:
            with open(p, "r", encoding="utf-8") as f: return json.load(f)
        except Exception: return {}
...
def _save_emisor_fiscal(data):
    Path(EMISOR_FISCAL_FILE).parent.mkdir(parents=True, exist_ok=True)
    with open(EMISOR_FISCAL_FILE, "w", encoding="utf-8") as f:
        json.dump(data, f, ensure_ascii=False, indent=2)

def _fiscalapi_personas_cache():
    p = Path(FISCALAPI_PERSONAS_CACHE_FILE)
    if p.exists():
        try:
            with open(p, "r", encoding="utf-8") as f: return json.load(f)
        except Exception: return {}
...
def _fiscalapi_personas_cache_set(rfc, persona_id):
    data = _fiscalapi_personas_cache()
    data[rfc.strip().upper()] = persona_id
    Path(FISCALAPI_PERSONAS_CACHE_FILE).parent.mkdir(parents=True, exist_ok=True)
    with open(FISCALAPI_PERSONAS_CACHE_FILE, "w", encoding="utf-8") as f:
        json.dump(data, f, ensure_ascii=False, indent=2)

def _fiscalapi_headers():
    return {
        "X-TENANT-KEY": _os.environ.get("FISCALAPI_TENANT_KEY", ""),
        "X-API-KEY": _os.environ.get("FISCALAPI_API_KEY", ""),
        "X-TIME-ZONE": "America/Mexico_City",
        "Content-Type": "application/json",
    }

def _fiscalapi_request(method, path, body=None):
    """Regresa (ok: bool, data_o_mensaje_de_error)."""
    if not _os.environ.get("FISCALAPI_API_KEY") or not _os.environ.get("FISCALAPI_TENANT_KEY"):
        return False, "Faltan configurar FISCALAPI_API_KEY / FISCALAPI_TENANT_KEY como variables de entorno en el servidor."
    try:
        url = FISCALAPI_BASE_URL.rstrip("/") + path
        resp = _requests.request(method, url, headers=_fiscalapi_headers(), json=body, timeout=25)
        try:
            data = resp.json()
        except Exception:
            data = {"raw": resp.text}
        ...
        return False, f"Error de conexión con FISCAL API: {e}"

def _fiscalapi_create_persona(datos):
    """⚠ Payload de mejor esfuerzo (no confirmado contra la documentación real de
    /persons). Si falla, usar el flujo manual (crear en su dashboard y pegar el ID)."""
    body = {
        "legalName": datos.get("razon_social", ""),
        "taxId": (datos.get("rfc") or "").strip().upper(),
        "email": datos.get("email", ""),
        "taxRegimeCode": datos.get("regimen_fiscal", ""),
        "cfdiUseCode": datos.get("uso_cfdi", ""),
        "zipCode": datos.get("codigo_postal", ""),
    }
    ok, data = _fiscalapi_request("POST", "/api/v4/persons", body)
    if not ok: return False, data
    pid = data.get("id") or (data.get("data") or {}).get("id")
    if not pid: return False, f"Respuesta sin ID reconocible: {data}"
    return True, pid

def _fiscalapi_create_producto(concepto):
    """⚠ Payload de mejor esfuerzo (no confirmado contra la documentación real de
    /products). Si falla, usar el flujo manual (crear en su dashboard y pegar el ID)."""
    body = {
        "description": concepto.get("descripcion", ""),
        "unitPrice": float(concepto.get("valor_unitario") or 0),
        "unitCode": concepto.get("clave_unidad") or "H87",
        "taxObjectCode": concepto.get("objeto_imp") or "02",
        "productCode": concepto.get("clave_prod_sat", ""),
    }
    ok, data = _fiscalapi_request("POST", "/api/v4/products", body)
    if not ok: return False, data
    pid = data.get("id") or (data.get("data") or {}).get("id")
    if not pid: return False, f"Respuesta sin ID reconocible: {data}"
    return True, pid

@app.route("/api/emisor-fiscal", methods=["GET"])
def api_get_emisor_fiscal():
    if not can("view", "fin-cpc"): return jsonify({"error": "Sin permiso"}), 403
    return jsonify(_load_emisor_fiscal())

@app.route("/api/emisor-fiscal", methods=["PUT"])
def api_set_emisor_fiscal():
    if not is_admin(): return jsonify({"error": "Sin permiso — solo administradores"}), 403
    data = request.json or {}
    emisor = {
        ...
        "fiscalapi_persona_id": (data.get("fiscalapi_persona_id") or "").strip(),
    }
    with lock:
        _save_emisor_fiscal(emisor)
    return jsonify({"ok": True, "emisor": emisor})

@app.route("/api/fiscalapi/crear-persona-receptor", methods=["POST"])
def api_fiscalapi_crear_persona_receptor():
    """Intenta crear (o usar caché local) la Persona-receptor en FISCAL API.
    Si falla, regresa el error para que el usuario la cree manualmente y pegue el ID."""
    if not can("create", "fin-cpc"): return jsonify({"error": "Sin permiso"}), 403
    datos = request.json or {}
    rfc = (datos.get("rfc") or "").strip().upper()
    if not rfc: return jsonify({"error": "Falta el RFC del receptor"}), 400

    manual_id = (datos.get("persona_id_manual") or "").strip()
    if manual_id:
        _fiscalapi_personas_cache_set(rfc, manual_id)
        return jsonify({"ok": True, "persona_id": manual_id, "origen": "manual"})

    cache = _fiscalapi_personas_cache()
    if rfc in cache:
        return jsonify({"ok": True, "persona_id": cache[rfc], "origen": "cache"})

    ok, result = _fiscalapi_create_persona(datos)
    if not ok:
        return jsonify({"error": result, "sugerencia": "Créala manualmente en FISCAL API (Catálogos → Personas) y pega su ID."}), 400
    _fiscalapi_personas_cache_set(rfc, result)
    return jsonify({"ok": True, "persona_id": result, "origen": "creado"})

@app.route("/api/cpc/<cpc_id>/timbrar-fiscalapi", methods=["POST"])
def api_cpc_timbrar_fiscalapi(cpc_id):
    """Genera y timbra el CFDI de una CPC con FISCAL API. Requiere que el emisor ya
    esté configurado (con su fiscalapi_persona_id) y que se pase el receptor + conceptos."""
    if not can("create", "fin-cpc"): return jsonify({"error": "Sin permiso"}), 403
    data = request.json or {}
    year = int(data.get("year", CURRENT_YEAR))

    emisor = _load_emisor_fiscal()
    if not emisor.get("fiscalapi_persona_id"):
        return jsonify({"error": "Falta configurar los Datos Fiscales del Emisor (RFC de Persico + su ID de Persona en FISCAL API). Ve a Configuración → Datos Fiscales del Emisor.", "step": "emisor"}), 400

    receptor = data.get("receptor") or {}
    rfc_receptor = (receptor.get("rfc") or "").strip().upper()
    if not rfc_receptor:
        return jsonify({"error": "Falta el RFC del receptor"}), 400

    receptor_id = (receptor.get("persona_id_manual") or "").strip() or _fiscalapi_personas_cache().get(rfc_receptor)
    if not receptor_id:
        ok, result = _fiscalapi_create_persona(receptor)
        if not ok:
            return jsonify({"error": f"No se pudo crear al receptor en FISCAL API: {result}",
                             "sugerencia": "Créalo manualmente en su dashboard (Catálogos → Personas) y pega su ID en el campo correspondiente.",
                             "step": "receptor"}), 400
        receptor_id = result
        _fiscalapi_personas_cache_set(rfc_receptor, receptor_id)
    ...
    items = []
    for c in conceptos:
        prod_id = (c.get("producto_id_manual") or "").strip()
        if not prod_id:
            ok, result = _fiscalapi_create_producto(c)
            if not ok:
                return jsonify({"error": f"No se pudo crear el concepto '{c.get('descripcion','')}' en FISCAL API: {result}",
                                 "sugerencia": "Créalo manualmente en su dashboard (Catálogos → Productos) y pega su ID.",
                                 "step": "producto", "concepto": c.get("descripcion")}), 400
            prod_id = result
        items.append({"id": prod_id, "quantity": float(c.get("cantidad") or 1), "discount": float(c.get("descuento") or 0)})

    body = {
        "versionCode": "4.0",
        "series": data.get("serie") or "F",
        "date": datetime.datetime.now().strftime("%Y-%m-%dT%H:%M:%S"),
        "paymentFormCode": data.get("forma_pago") or "99",
        "paymentMethodCode": data.get("metodo_pago") or "PUE",
        "currencyCode": data.get("moneda") or "MXN",
        "typeCode": "I",
        "expeditionZipCode": emisor.get("codigo_postal", ""),
        "exchangeRate": 1,
        "exportCode": "01",
        "issuer": {"id": emisor["fiscalapi_persona_id"]},
        "recipient": {"id": receptor_id},
        "items": items,
    }
    ok, result = _fiscalapi_request("POST", "/api/v4/invoices", body)
    if not ok:
        return jsonify({"error": f"FISCAL API rechazó la factura: {result}", "step": "timbrado"}), 400

    payload = result.get("data") if isinstance(result, dict) and "data" in result else result
    invoice_id = (payload or {}).get("id")
    uuid_timbre = (payload or {}).get("uuid") or (payload or {}).get("stampUuid") or (payload or {}).get("satUuid")

    with lock:
        records = cpc_load(year)
        rec = next((r for r in records if r.get("id") == cpc_id), None)
        if not rec:
            return jsonify({"error": "CPC no encontrada (la factura sí se generó en FISCAL API, guarda el ID manualmente)", "fiscalapi_response": result}), 404
        rec["fiscalapi_invoice_id"] = invoice_id
        rec["fiscalapi_uuid"] = uuid_timbre
        rec["fiscalapi_estatus"] = "Timbrada"
        rec["fiscalapi_raw"] = result
        if uuid_timbre: rec.setdefault("cfdi", {})["uuid"] = uuid_timbre
        cpc_save(year, records)

    return jsonify({"ok": True, "record": rec, "fiscalapi_response": result})
