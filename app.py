...
<div class="header">
  <div class="company-name">PERSICO MÉXICO S.A. DE C.V.</div>
  <div>
    <div class="pur-num-label">Procesar Compra</div>
    <div class="pur-num">{pur['pur_number']}</div>
    <div style="font-size:10px;color:#888;text-align:right;margin-top:4px">{pur['fecha']}</div>
  </div>
</div>
<div class="meta-grid">
  <div class="meta-field"><b>Recepción (REC)</b>{pur.get('rec_number','—')}</div>
  <div class="meta-field"><b>Orden de Compra</b>{pur.get('po_number','—')}</div>
  <div class="meta-field"><b>No. Factura</b>{pur.get('factura','—')}</div>
  <div class="meta-field"><b>Proveedor</b>{pur.get('supplier_name','—')}</div>
  <div class="meta-field"><b>Procesado por</b>{pur.get('usuario','—')}</div>
</div>
<div class="total-row">
  <span>MONTO&nbsp;&nbsp;{fmt(pur.get('monto',0))}</span>
</div>
<div class="footer">
  <span>Generado por Persico Suite · {datetime.datetime.now().strftime('%Y-%m-%d %H:%M')}</span>
  <span>{pur['pur_number']} · {pur.get('supplier_name','')}</span>
</div>
</body></html>"""
        return Response(html, mimetype="text/html",
    ...
    try:
        ...
        with lock:
            ...
            for cpp_number in cpp_numbers:
                ...
                pago = {
                    "pago_number":   _doc_next_number("PAG"),
