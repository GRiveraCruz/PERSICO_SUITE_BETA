...
def api_create_recepcion():
    """
    Registra una recepción de compra (Finanzas).
    Body: {
      po_number, compra_directa (bool), tipo_recepcion ('completa'|'parcial'),
      supplier {nombre, rfc, ...} (usado si compra_directa=true, o snapshot de la OC),
      factura, notes,
      items: [{part_number, description, brand, job, quantity_received, unit_cost}]
    }
    """
    try:
        ...
        tipo_rec      = str(data.get("tipo_recepcion","")).strip().lower()
        ...
        with lock:
            ...
            gpo_rec = None
            if not compra_directa and po_number:
                gpos = gpo_load()
                ...
                # Actualizar cantidades entregadas en la GPO y su IPO
                for it in rec_items:
                    match = next((gi for gi in gpo_rec.get("items",[])
                        if str(gi.get("part_number","")).strip().upper()==it["part_number"]), None)
                    if match:
                        prev = float(match.get("quantity_delivered",0) or 0)
                        match["quantity_delivered"] = prev + it["quantity_received"]
                all_delivered = all(
                    float(gi.get("quantity_delivered",0)) >= float(gi.get("quantity",1))
                    for gi in gpo_rec.get("items",[]))
                any_delivered = any(
                ...
                gpo_rec["status"] = "Entregada" if all_delivered else ("Parcial" if any_delivered else "Emitida")
                gpo_save(gpos)
                # Reflejar estatus en IPO
                year = datetime.datetime.now().year
                ipo_recs = po_load(year)
                for ir in ipo_recs:
                    if str(ir.get("gpo_number","")).upper()==po_number:
                        ir["estatus"] = gpo_rec["status"]
                po_save(year, ipo_recs)
            ...
            rec = {
                ...
                "tipo_recepcion":tipo_rec or ("completa" if not compra_directa else ""),
                "supplier":      supplier,
                "supplier_name": str(supplier.get("nombre","")).strip().upper(),
                "factura":       factura,
                "notes":         notes,
...
<div class="meta-bar">
  ...
  <div><span>Tipo</span> <b>{'Compra Directa' if rec.get('compra_directa') else rec.get('tipo_recepcion','').upper()}</b></div>
