...
TIPOS_PERMISO = ["Con goce de sueldo", "Sin goce de sueldo", "Incapacidad (IMSS)", "Falta justificada"]
...
def api_create_permiso():
    ...
    if not tid or tipo not in TIPOS_PERMISO or not fecha_inicio or not fecha_fin:
        return jsonify({"error": "Faltan datos obligatorios (trabajador, tipo, fechas)"}), 400
...
def api_update_permiso(pid):
    ...
    with lock:
        ...
        tipo = (data.get("tipo") or rec["tipo"]).strip()
        if tipo not in TIPOS_PERMISO:
            return jsonify({"error": "Tipo de permiso inválido"}), 400
        fecha_inicio = (data.get("fecha_inicio") or rec["fecha_inicio"]).strip()
        fecha_fin = (data.get("fecha_fin") or rec["fecha_fin"]).strip()
        if fecha_fin < fecha_inicio:
            return jsonify({"error": "La fecha de fin no puede ser anterior a la de inicio"}), 400
        rec["tipo"] = tipo
        rec["fecha_inicio"] = fecha_inicio
        rec["fecha_fin"] = fecha_fin
        rec["dias"] = _dias_permiso(fecha_inicio, fecha_fin)
        rec["motivo"] = (data.get("motivo", rec.get("motivo")) or "").strip() or None
        _save_permisos(records)
    return jsonify(rec)
    ...
    with lock:
        ...
        if etapa == "jefe":
            ...
            if decision == "aprobado":
                ...
                if rec["tipo"] == "Con goce de sueldo" and not rec.get("vacaciones_descontado"):
                    _add_dias_gozados_nolock(rec["tid"], rec["dias"], nota=f'Permiso {rec["id"]} ({rec["fecha_inicio"]} a {rec["fecha_fin"]})')
